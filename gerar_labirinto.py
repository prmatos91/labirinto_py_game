...
def avancar(tela, jogadas, todas_jogadas, rgb, ultima_jogada, x, y):
	...
	for c in range(4):
		...
		if c == 0 and r != 0 and ultima_jogada != "D" and j[0]+[[d_fim[0][0]-3,d_fim[0][1]]] not in todas_jogadas:
			...
			ultima_jogada = "E"
			break
		elif c == 1 and r != 0 and ultima_jogada != "C" and j[0]+[[d_fim[1][0],d_fim[1][1]+3]] not in todas_jogadas:
			var = [d_fim[1][0],d_fim[1][1]+3]
			pygame.draw.line(tela, (255,0,0), (j[0][-1]), var)
			ultima_jogada = "B"
			# add = True
			break
		elif c == 2 and r != 0 and ultima_jogada != "E" and j[0]+[[d_fim[2][0]+3,d_fim[2][1]]] not in todas_jogadas:
			var = [d_fim[2][0]+3,d_fim[2][1]]
			pygame.draw.line(tela, (255,0,0), (j[0][-1]), var)
			ultima_jogada = "D"
			break
		elif c == 3 and r != 0 and ultima_jogada != "B" and j[0]+[[d_fim[3][0],d_fim[3][1]-3]] not in todas_jogadas:
